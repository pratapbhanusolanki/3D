...
for i=2:num_iteration
    ...
    if i > 3
        ...
        h1 = quiver3(xp(i-1) ,yp(i-1),zp(i-1),xp(i)- xp(i-1),yp(i)- yp(i-1),zp(i)- zp(i-1) ...
            ,'-b','MaxHeadSize',5,'ShowArrowHead','on','AutoScale','off' );
        h1 = plot3([xp(i-1) xp(i)],[yp(i-1) yp(i)], [zp(i-1) zp(i)],'-b');
    end

    h2 = plot3([ xa],[ ya],[ za],'rs','MarkerSize',15);

    h3 = plot3([0 xe],[0 ye],[0 ze],':go','MarkerSize',5);
...
legend('Simulated measurement','Estimated measurement','x_1 - reference')
