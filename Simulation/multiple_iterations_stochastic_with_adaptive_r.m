...
scan_radii = [1,2,5,10,15,20,25,30,35];
%scan_radii = 35;
scan_phii = [10,20,30,40,50,60,70,80,90];
p_norm_mesh_mean_all = zeros(length(scan_radii),length(scan_phii));
control_effort_all = zeros(length(scan_radii),length(scan_phii));
difference_all = zeros(length(scan_radii),length(scan_phii));
%hFig = figure;
...
noise_measurement = normrnd(zeros(num_iteration,1),R_system(1,1));
    ...
    for ir=1:length(scan_radii)
        ...
        actual_position(:,1) = [-4,40,40]'; % -4 so as to keep x1 positive 
        ...
        control_effort(ir,i) = sqrt(Motor_command_psi^2 + Motor_command_theta^2);
        %My position
        azimuth = my_pos(2,i);
    ...
    %     h3 = plot3([0 xe],[0 ye],[0 ze],'--go','MarkerFaceColor','g','LineWidth',1);
        ...
        control_effort_total(ir,ip) = sum(control_effort(ir,stability_limit+1:num_iteration));
        difference_total(ir,ip) = sum(diff_en(ir,stability_limit+1:num_iteration));
    ...
    p_norm_mesh_mean_all =  p_norm_mesh_mean_all + p_norm_mesh_mean;
    control_effort_all = control_effort_all + control_effort_total;
    difference_all = difference_all + difference_total;
    p_norm_dummy{n} = p_norm_mesh_mean;
end
...
p_norm_mesh_mean_all = p_norm_mesh_mean_all/num_tries;
control_effort_all = control_effort_all/num_tries;
surf(X,Y,p_norm_mesh_mean_all);
xlabel('\phi (degree)');
ylabel('r (degree)');
zlabel('||P||_2');

figure;
surf(X,Y,control_effort_all);
xlabel('\phi (degree)');
ylabel('r (degree)');
zlabel('U');
